...
RUN sed -i 's/192.168.1.1/192.168.133.78/g' $homedir/ebot-csgo-web/config/app_user.yml
...
CMD ["sh", "-c", "sleep 10 ; /usr/bin/php symfony configure:database 'mysql:host=mysql;dbname=ebotv3' ebotv3 ebotv3 ; php symfony doctrine:insert-sql; php symfony guard:create-user --is-super-admin admin@ebot admin password ; php symfony cc ; rm -rf $homedir/ebot-csgo-web/web/installation ; apachectl -d /etc/apache2 -f /etc/apache2/apache2.conf -e debug -DFOREGROUND"]
